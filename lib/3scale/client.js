var AuthorizeResponse, Client, Response, exports, http, parserxml, querystring, sys;
http = require('http');
sys = require('sys');
    ...
    return request.on('response', function(response) {
      ...
      return response.on('end', function() {
        switch (response.statusCode) {
          ...
          default:
            throw "[Client::authorize] Server Error";
        }
      });
    });
  };
  Client.prototype.report = function(trans) {
    var obj, query, request, threescale, url;
    if (trans == null) {
    ...
    return request.on('response', function(response) {
      ...
      return sys.log(response.statusCode);
    });
  };
